...
class PlaneWars:
    def __init__(self):
        ...
        pygame.time.set_timer(
            definitions.ENEMY_SPAWN_EVENT, definitions.ENEMY_SPAWN_TIME
        )
    ...
    def start_game(self):
        while True:
            ...
            pygame.display.set_caption(
                "Plane wars | FPS: " + str("{:.2f}".format(self.clock.get_fps()))
            )
    ...
    def __event_handler(self):
        ...
        if not keys_pressed[pygame.K_SPACE]:
            pygame.time.set_timer(
                definitions.HERO_FIRE_EVENT, definitions.HERO_FIRE_INTERVAL
            )
